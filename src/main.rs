...
fn main() {
    ...
    let message = b"sylvainpelissier";

    /* Enter solution here */

    let mut new_key = G1Projective::zero();
    let mut new_proof = G2Projective::zero();
    ...
    for n in 0..public_keys.len() {
        let tmp = index.inverse().unwrap() * ten;
        new_proof = new_proof.add(public_keys[n].1.mul(tmp));
        index = index + one;
    }
    let new_proof = new_proof.neg().into_affine();
